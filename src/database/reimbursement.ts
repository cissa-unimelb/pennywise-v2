...
export type DepartmentEnum =
  ...
  | "Publicity"
  | "Product";
export type StatusEnum = "Active" | "Approve" | "Reject" | "Archived";
...
export async function getMyReimbursement(
  user: User,
): Promise<ReimbursementRead[]> {
  const q = query(
    collection(db, "reimbursement"),
...
export async function updateReimbursement(
  docId: string,
  updateData: Partial<Reimbursement>,
) {
  await updateDoc(doc(db, "reimbursement", docId), { ...updateData });
}

export async function getArchivedReimbursements(
  user: User,
): Promise<ReimbursementRead[]> {
  const q = query(
    collection(db, "reimbursement"),
  ...
  return snapShotToList(await getDocs(q));
}
